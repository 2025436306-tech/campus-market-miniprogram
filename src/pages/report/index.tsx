...
export default function Report() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Taro.hideLoading();
      Taro.showToast({ title: '感谢反馈，我们将尽快核实', icon: 'none', duration: 2000 });
  ...
  return (
    // 核心修复：加入 w-full 和 box-border 限制宽度，完美居中
    <View className="flex flex-col h-screen bg-[#f4f4f5] w-full box-border">
      <ScrollView scrollY className="flex-1 p-5 w-full box-border">
      ...
      {/* 底部约束宽度 */}
      <View className="bg-white p-4 pb-8 border-t border-zinc-100 shrink-0 z-50 w-full box-border">
        <Button className="w-full bg-blue-600 text-white rounded-full font-bold py-1 border-none shadow-lg shadow-blue-500/30 active:opacity-90" onClick={handleSubmit}>
          提交举报
        </Button>
      </View>
    </View>
